...
// GNSS params
    ...
    constexpr unsigned long timeout_first_fix_seconds = 1UL * 60UL;  // a reasonable value
    // put 6UL * 60UL * 60UL for a sleep if no initial fix of 6 hours
    constexpr long sleep_no_initial_fix_seconds = 30L;  // for testing

    // parameter for timeout of later (i.e. non first) GNSS fixes
    // put 3UL * 60UL for a later fix timeout of 3 minutes
    constexpr unsigned long timeout_gnss_fix_seconds = 1UL * 60UL;
    ...
    constexpr long interval_between_gnss_measurements_seconds = 15L * 60L;  // a reasonable value
    ...
    constexpr size_t min_nbr_of_fix_per_message = 10;  // a reasonable value
...
constexpr unsigned long timeout_cap_charging_seconds = 2UL * 60UL;

// for how long we retry transmitting a given message
constexpr int timeout_attempt_transmit_seconds {180};
...
constexpr size_t fft_length {1024};
...
constexpr size_t total_number_of_samples {1024 * 1};
// constexpr size_t total_number_of_samples {2048 * 1};
...
constexpr size_t fft_overlap {1024};
// constexpr size_t fft_overlap {2048};
...
constexpr size_t number_welch_segments {1};
...
constexpr size_t welch_bin_max {16};
...
constexpr long interval_between_wave_spectra_measurements {15 * 60};
// tolerance in seconds for jitter; typically 5 minutes should be more than enough
constexpr long tolerance_seconds_start_wave_measurements {1 * 60};
